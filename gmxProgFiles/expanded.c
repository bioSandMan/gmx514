...
static int ChooseNewLambda(int nlim, t_expanded *expand, df_history_t *dfhist, int fep_state, real *weighted_lamee, double *p_k,
                           gmx_int64_t seed, gmx_int64_t step, t_lambda *fep)
{
    ...
    for (i = 0; i < expand->lmc_repeats; i++)
    {
        ...
        else if ((expand->elmcmove == elmcmoveMETROPOLIS) || (expand->elmcmove == elmcmoveBARKER) || (expand->elmcmove == elmcmoveAIM))
        {
            ...
            else if (expand->elmcmove == elmcmoveAIM)
            {
                real df = 0.0;
                int j;
                for (j=0; j < efptNR; j++)
                {
                    if (fep->separate_dvdl[j])
                    {
                        df += 0.5
                           ...
                           * (dfhist->avg_dvdl[j][lamtrial] + dfhist->avg_dvdl[j][fep_state])
    ...
    if (mod(step, frequency) == 0)
    {
        ...
        else if (expand->elmcmove == elmcmoveAIM)
        {
            ...
            for (i = 0; i < efptNR; i++)
            {
                if (fep->separate_dvdl[i])
                {
                    fprintf(outfile, " dGd[%s]", print_names[i]);
                }
            }
            fprintf(outfile, "    \n");
        }
        ...
        for (ifep = 0; ifep < nlim; ifep++)
        {
            ...
            {
                fprintf(outfile, " %8d", (expand->elmcmove == elmcmoveAIM)? dfhist->aim_at_lam[ifep] : dfhist->n_at_lam[ifep]);
            }
            if (expand->elamstats == elamstatsMINVAR)
            {
            ...
            if (expand->elmcmove == elmcmoveAIM)
            {
                for (i = 0; i < efptNR; i++)
                {
                    if (fep->separate_dvdl[i])
                    {
                        fprintf(outfile, "%10.5f", dfhist->avg_dvdl[i][ifep]);
                    }
                }
            }
            if (ifep == fep_state)
   ...
   Hamiltonian expanded ensemble.  May be able to remove it after integrator refactoring. */
{
    ...
    lamnew = ChooseNewLambda(nlim, expand, dfhist, fep_state, weighted_lamee, p_k,
                             ir->expandedvals->lmc_seed, step, ir->fepvals);
    ...
    // if performing AIM, we need to update the averages
    if (expand->elmcmove == elmcmoveAIM)
    {
        // update the count at the current lambda
        // we do this here because n_at_lambda gets reset
        // if wang-landau is used
        dfhist->aim_at_lam[fep_state]++;

        // delta helps dealing with large values
        double delta = 0.0;
        for (i = 0; i < efptNR; i++)
        {
            ...
            {
                delta = enerd->term[F_DVDL+i] - dfhist->avg_dvdl[i][fep_state];
                dfhist->avg_dvdl[i][fep_state] += delta/dfhist->aim_at_lam[fep_state];
            }
        }
